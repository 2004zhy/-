...
bool update_bmpFile(const char* filename) {
    ...
    for (w = 0; w < my_bmp.col; w++) {
        for (h = my_bmp.row - 1; h >= 0; h--) {
            for (int i = 0; i < 3; i++) {
                // 应用黑参考校正
                int raw_value = pdata[h * pitch + w * BytePerPix + i];
                int corrected_value = raw_value - (*black_level)[w][i];
                ...
                if (corrected_value > 255) {
                    pdata[h * pitch + w * BytePerPix + i] = 255;
                }
                else if (corrected_value < 0) {
                    pdata[h * pitch + w * BytePerPix + i] = 0;
                }
                else {
                    pdata[h * pitch + w * BytePerPix + i] = (U8)corrected_value;
                }
            }
        }
    }
...
void update_weight(void) {
    // 初始化白参考权重
    //std::vector<std::vector<double>> white_weight = init_weight("/home/root/cis_app/white.bmp");
    // 初始化黑参考权重
    //std::vector<std::vector<double>> black_weight = init_weight_black("/home/root/cis_app/black.bmp");
    if (change_scan == false) {
